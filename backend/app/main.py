...
async def get_static_data():
    """
    Generate randomized static profile data for demo/testing using Python's random library.
    Since the platform is new, this provides mock users for testing the chat interface.
    """
    import random
    
    # Sample data for generating realistic mock profiles
    ADJECTIVES = ["Happy", "Quick", "Clever", "Bright", "Calm", "Cool", "Swift", "Witty", "Smart", "Keen", 
                  "Sunny", "Lucky", "Merry", "Jolly", "Eager", "Ready", "Brave", "Noble", "Kind", "Free"]
    NOUNS = ["Panda", "Phoenix", "Tiger", "Eagle", "Fox", "Wolf", "Owl", "Hawk", "Bear", "Dragon", 
    ...
    # Generate random daily limits
    daily_limits = {
        "male": random.randint(3, 7),
        "female": random.randint(3, 7),
        "non-binary": random.randint(3, 7),
        "prefer-not-to-say": random.randint(3, 7)
    }
    
    # Status and remaining matches
    status = random.choice(["ONLINE", "IDLE"])
    
    return {
        "device_id": f"mock_{random.randint(100000, 999999)}",
        "profile": {
            "nickname": nickname,
            "bio": bio,
            "gender": gender,
        ...
        "remaining_matches": {
            "male": random.randint(1, daily_limits["male"]),
            "female": random.randint(1, daily_limits["female"]),
            "non-binary": random.randint(1, daily_limits["non-binary"]),
            "prefer-not-to-say": random.randint(1, daily_limits["prefer-not-to-say"])
        }
    }
